# FullSetB_1 reproduction

This example reproduces Fredi's SPS H2 `FullSetB_1` merged-muon calibration
...
The merged input is `Muon_FullSetB_1`. This set uses the V2 summing-board
...
```text
calibrations/TB2026/calib_SPS-H2_FullSetB_1.txt
```
...
```tcsh
cd "$LFHCAL_REPO/examples/yall/fullset-b1-repro"
source env.tcsh
yall-run validate
yall-run plan
...
Results go under `$LFHCAL_WORK/fullset-b1-repro/`. The final text
calibration is
`final/calib_Final_Muon_FullSetB_1_calib.txt`.
...
```tcsh
python3 ../compare_fullset.py --set-name FullSetB_1
```
