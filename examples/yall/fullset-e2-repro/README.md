# FullSetE_2 reproduction

This example reproduces Fredi's SPS H2 `FullSetE_2` merged-muon calibration
...
The merged input is `Muon_FullSetE_2`. This set uses the V2 summing-board
...
```text
calibrations/TB2026/calib_SPS-H2_FullSetE_2.txt
```
...
```tcsh
cd "$LFHCAL_REPO/examples/yall/fullset-e2-repro"
source env.tcsh
yall-run validate
yall-run plan
...
Results go under `$LFHCAL_WORK/fullset-e2-repro/`. The final text
calibration is
`final/calib_Final_Muon_FullSetE_2_calib.txt`.
...
```tcsh
python3 ../compare_fullset.py --set-name FullSetE_2
```
