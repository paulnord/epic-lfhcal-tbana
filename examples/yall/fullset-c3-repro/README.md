# FullSetC_3 reproduction

This example reproduces Fredi's SPS H2 `FullSetC_3` merged-muon calibration
...
The merged input is `Muon_FullSetC_3`. This set uses the V2 summing-board
mapping, the shared FullSetA-F bad-channel map, and
`configs/TB2026/ToAOffsets_TBSPS2026_FullSetC_2.csv`.

Published reference:

```text
calibrations/TB2026/calib_SPS-H2_FullSetC_3.txt
```
...
```tcsh
cd "$LFHCAL_REPO/examples/yall/fullset-c3-repro"
source env.tcsh
yall-run validate
yall-run plan
...
Results go under `$LFHCAL_WORK/fullset-c3-repro/`. The final text
calibration is
`final/calib_Final_Muon_FullSetC_3_calib.txt`.
...
```tcsh
python3 ../compare_fullset.py --set-name FullSetC_3
```
