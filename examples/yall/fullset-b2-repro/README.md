# FullSetB_2 reproduction

This example reproduces Fredi's SPS H2 `FullSetB_2` merged-muon calibration
...
The merged input is `Muon_FullSetB_2`. This set uses the V2 summing-board
mapping, the shared FullSetA-F bad-channel map, and
`configs/TB2026/ToAOffsets_TBSPS2026_FullSetB.csv`.

Published reference:

```text
calibrations/TB2026/calib_SPS-H2_FullSetB_2.txt
```
...
```tcsh
cd "$LFHCAL_REPO/examples/yall/fullset-b2-repro"
source env.tcsh
yall-run validate
yall-run plan
...
Results go under `$LFHCAL_WORK/fullset-b2-repro/`. The final text
calibration is
`final/calib_Final_Muon_FullSetB_2_calib.txt`.
...
```tcsh
python3 ../compare_fullset.py --set-name FullSetB_2
```
