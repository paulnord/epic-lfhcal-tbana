...
struct Config {
  ...
  std::optional<double> start_width;
  std::optional<double> start_mp;
  std::optional<double> start_area;
  std::optional<double> start_gsigma;
...
[[noreturn]] void usage(const char *argv0, int code = 2) {
  ...
  out
      ...
      << "  --start-width X       override only Landau-width starting value\n"
      << "  --start-mp X          override only MP starting value\n"
      << "  --start-area X        override only area starting value\n"
      << "  --start-gsigma X      override only Gaussian-sigma starting value\n"
      ...
      << "Explicit starting values must lie strictly inside their final parameter limits.\n\n"
...
Config parse_args(int argc, char **argv) {
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    else if (arg == "--start-width") c.start_width = parse_double(need(i, "--start-width"), "start-width");
    else if (arg == "--start-mp") c.start_mp = parse_double(need(i, "--start-mp"), "start-mp");
    else if (arg == "--start-area") c.start_area = parse_double(need(i, "--start-area"), "start-area");
    else if (arg == "--start-gsigma") c.start_gsigma = parse_double(need(i, "--start-gsigma"), "start-gsigma");
...
FitSetup production_setup(TH1 &h, const Config &c, const Seed &seed) {
  ...
  // Absolute starting-value overrides are applied last. In particular, area
  // starts do not redefine normalization bounds, and MP starts do not replace
  // the calibration seed used to derive the fit interval and parameter limits.
  const std::optional<double> starts[]{c.start_width, c.start_mp,
                                       c.start_area, c.start_gsigma};
  const char *names[]{"--start-width", "--start-mp", "--start-area", "--start-gsigma"};
  for (int i=0; i<4; ++i) {
    if (!starts[i]) continue;
    if (!std::isfinite(*starts[i]) ||
        !(*starts[i] > s.low[i] && *starts[i] < s.high[i])) {
      throw std::runtime_error(std::string(names[i]) +
                               " must be finite and strictly inside its parameter bounds");
    }
    s.start[i] = *starts[i];
  }
