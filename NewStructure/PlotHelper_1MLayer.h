...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/**
 * Plot noise spectra with fits for a single 1M layer.
 *
 * Draws the selected noise histogram (HG/LG/TOA/TOT) for a single cell or
 * layer depending on the provided layer/mod/extCellId arguments. When available
 * fit TF1 objects are drawn and annotated. The helper is a drawing-only
 * utility and expects a prepared canvas; it will call canvas->SaveAs(nameOutput)
 * after rendering.
 */
                                     ...
                                     Double_t xPMin, Double_t xPMax, Double_t scaleYMax, int layer, int mod, TString nameOutput, RunInfo currRunInfo, int extCellId = -1)
{
  ...
  if (maxY < FindLargestBin1DHist(tempHist, xPMin, xPMax))
    maxY = FindLargestBin1DHist(tempHist, xPMin, xPMax);
...
/**
 * Plot spectra (HG/LG/combined/TOA/TOT) with optional fits for a single 1M layer.
 *
 * Draws one histogram (selected by `option`) for a single cell or for the
 * specified layer/module combination. Applies consistent styling, optional
 * fit overlays and draws per-panel labels. Side effects: modifies the canvas
 * state and may call canvas->SaveAs(nameOutput).
 */
...
/**
 * Plot 2D correlations / TH2 maps for a single 1M layer.
 *
 * This helper draws TH2D correlation maps (or profile-like visualizations)
 * for the specified cell/layer and applies calibration/bad-channel boxes when
 * requested. It configures log/z-scaling as appropriate and may call
 * canvas2Panel->SaveAs(nameOutput) once panels are rendered.
 */
                                  ...
                                  Double_t xPMin, Double_t xPMax, int layer, int mod, TString nameOutput, TString nameOutputSummary, RunInfo commonRunInfo, Int_t detailedPlot = 1, bool plotMean = false, int labelOpt = 0, int extCellId = -1)
{
  ...
  if (optionTrend == 0) { // HG
    if (maxY < ithTrend->second.GetMaxHGSpec())
      maxY = ithTrend->second.GetMaxHGSpec();
    if (minY > ithTrend->second.GetMinHGSpec())
      minY = ithTrend->second.GetMinHGSpec();
  }  else if (optionTrend == 1) { // LG
    if (maxY < ithTrend->second.GetMaxLGSpec())
      maxY = ithTrend->second.GetMaxLGSpec();
    if (minY > ithTrend->second.GetMinLGSpec())
      minY = ithTrend->second.GetMinLGSpec();
  }
                                         ...
                                         RunInfo commonRunInfo, Int_t detailedPlot = 1, bool scaleInt = false, int labelOpt = 0, int extCellId = -1)
{
  ...
  for (int rc = 0; rc < ithTrend->second.GetNRuns() && rc < 30; rc++)  {
    ...
    if (tmpRunNr != -1) {
      ...
      if (scaleInt && profs[rc] != nullptr)
        profs[rc]->Scale(1 / profs[rc]->Integral());
    ...
    if (profs[rc]) {
      if (rc == 0) {
        ...
        if (scaleInt)
          yTitle = Form("%s/ integral", yTitle.Data());
...
//__________________________________________________________________________________________________________
// Plot Trending for 1 tile for all runs available
//__________________________________________________________________________________________________________
inline void PlotTrending1MLayer( TCanvas *canvas2Panel,  Double_t topRCornerX, Double_t topLCornerX,  Double_t topRCornerY,  Double_t relSizeP,  Int_t textSizePixel,
                                 std::map<int, TileTrend> trending,  int optionTrend,
                                 Double_t xPMin,  Double_t xPMax, Double_t minY, Double_t maxY, int isSameVoltage, double commonVoltage, int layer, int mod,
                                 TString nameOutput, RunInfo currRunInfo, Int_t extCellId = -1)
{
  Setup *setup = Setup::GetInstance();

  int tempCellID = -1;
  if (layer != -1 && mod != -1)
    tempCellID = setup->GetCellID(0, 0, layer, mod);
  ...
  auto ithTrend = trending.find(tempCellID);
  if (ithTrend == trending.end()) {
    std::cout << "WARNING: skipping cell ID: " << tempCellID << "\t layer " << layer << "\t module " << mod << std::endl;
    return;
  }
  ...
  canvas2Panel->cd();
  canvas2Panel->SetLogy(optionTrend == 6);
  ...
  TString yAxisTitle = tempGraph->GetYaxis()->GetTitle();
  if (tempGraph2) {
    yAxisTitle.ReplaceAll("(arb. units)", "");
    yAxisTitle += ", ";
    yAxisTitle += tempGraph2->GetYaxis()->GetTitle();
  }

  TH1D *dummyHist = new TH1D("dummyHist", "", 100, xPMin, xPMax);
  SetStyleHistoTH1ForGraphs(dummyHist, tempGraph->GetXaxis()->GetTitle(), yAxisTitle, 0.85 * textSizePixel, textSizePixel, 0.85 * textSizePixel, textSizePixel, 0.95, 1.4, 510, 510, 43, 63);
  ...
  TString cellLabel = Form("layer %d, module %d", layer, mod);

  if (extCellId != -1){
    cellLabel = Form( "a:%d, ch:%d",setup->GetROunit(tempCellID), setup->GetROchannel(tempCellID));
  }

  DrawLatex( topRCornerX , topRCornerY - 0.85* relSizeP, cellLabel, true, 0.85 * textSizePixel, 43);

  if (isSameVoltage > 0) {
    TString voltageLabel = Form(
        "Common V_{op} = %2.1f V", commonVoltage);
    if (isSameVoltage > 1)
      voltageLabel += Form( ",%s", GetSpeciesStringFromPDG(currRunInfo.pdg).Data());
    if (isSameVoltage > 2)
      voltageLabel += Form(", Run %d", currRunInfo.runNr);
    DrawLatex( topRCornerX, topRCornerY - 2*0.85*relSizeP, voltageLabel, true, 0.85 * textSizePixel, 43);
  }

  TString beamline = GetStringFromRunInfo(currRunInfo, 9);
  TString lab1 = Form("#it{#bf{%s TB:}} %s", currRunInfo.detector.Data(), beamline.Data());
  if (beamline.Contains("ORNL"))
    lab1 = beamline;

  DrawLatex( topLCornerX, topRCornerY - 0.85*relSizeP, lab1, false, 0.85 * textSizePixel, 43);
  DrawLatex(topLCornerX, topRCornerY - 2*0.85*relSizeP, GetStringFromRunInfo(currRunInfo, 8), false, 0.85 * textSizePixel, 43);
  DrawLatex(topLCornerX, topRCornerY - 3*0.85* relSizeP, GetStringFromRunInfo(currRunInfo, 10), false, 0.85 * textSizePixel, 43);

  if (tempGraph2){
    TLegend *legend = GetAndSetLegend2(0.3, 0.8, 0.6, 0.95, 0.85 * textSizePixel, 2, "", 43, 0.12);
