...
bool TileSpectra::FitMipHG( double* out, double* outErr,
                            ...
                            bool impE, double vov, double avmip){
  bmipHG = false;
  if (calib->BadChannel != -64 && calib->BadChannel < 2) {
    if (verbosity > 0)
      std::cout << "==========> Skipped HG cell " << cellID
                << " channel dead" << std::endl;
    return false;
  }
  if (ROType == ReadOut::Type::Hgcroc) {
    return lfhcal::adaptive::fitMipHG(*this, out, outErr, verbosity, year,
                                      impE, vov, avmip);
  }
  return fitMipHGLegacy(out, outErr, verbosity, year, impE, vov, avmip);
}

bool TileSpectra::fitMipHGLegacy( double* out, double* outErr, 
                            int verbosity, int year, 
                            bool impE = false, double vov = -1000, double avmip = -1000){
